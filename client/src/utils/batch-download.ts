...
import type { SourceDemandItem, FormFieldDefinition, FileAttachment } from '@shared/api.interface';
...
export function collectImagePaths(
  ...
  const imageFields = formFields?.filter((f: FormFieldDefinition) => f.type === 'image') ?? [];
    ...
    if (d.customFields) {
      for (const f of imageFields) {
        ...
        if (v && typeof v === 'object' && 'filePath' in v) {
          items.push({ name: `${base}_${sanitize(f.label)}`, filePath: (v as FileAttachment).filePath });
        }
      }
    }
  }
...
export async function downloadFilesAsZip(items: ImageDownloadItem[], zipName: string): Promise<void> {
  if (items.length === 0) {
    toast.warning('没有可下载的图片');
    return;
  }
  toast.info(`正在准备 ${items.length} 个文件...`);
  const bucket = (await getDataloom()).storage.from(getDefaultBucketId());
  const zip = new JSZip();
  ...
  const urls = items.map((i: ImageDownloadItem) => ({
    name: i.name,
    url: bucket.generateDownloadUrlFromFilePath(i.filePath),
  }));
  await runPool(urls, CONCURRENCY, async (u: { name: string; url: string }) => {
    try {
      const res = await fetch(u.url);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const blob = await res.blob();
      const ext = extractExt(u.url) || BLOB_EXT_MAP[blob.type] || 'jpg';
      const fn = dedupe(`${u.name}.${ext}`, used);
      zip.file(fn, blob);
      ok++;
    } catch (e) {
      fail++;
      logger.error(`下载失败: ${u.name}`, e instanceof Error ? e.message : String(e));
    }
  });
  if (ok === 0) {
...
function sanitize(name: string): string {
  return name.replace(/[/\\:*?"<>|]/g, '_').trim().slice(0, 50) || '未命名';
}
